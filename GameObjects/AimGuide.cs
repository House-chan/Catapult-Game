    ...
    class AimGuide : GameObject
    {
        ...
        public void Draw(SpriteBatch spriteBatch, Ship player, List<Planet> planet, int BulletType)
        {
            switch (BulletType)
            {
                ...
                case 1:
                    DrawForDefault(spriteBatch, player, planet, 3.0f);
                    break;
        ...
        public void DrawForDefault(SpriteBatch spriteBatch, Ship player, List<Planet> planet, float bullet_mass)
        {
            if (player.stage == Ship.Stage.Start || player.stage == Ship.Stage.Shooting)
            {
                shootpower = player.ShootPower;
                ...
                for (int i = 0; i < guide_length * 2; i++)
                {
                    ...
                    cur_velocity = Gravity.CalVelocity(position, cur_velocity, bullet_mass, planet);
                }
            }
        }
